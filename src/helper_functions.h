...
inline double multiv_prob(double sig_x, double sig_y, double x_tobs, double y_tobs, double mu_x, double mu_y){
  ...
  double exponent; 
  exponent = (pow(x_tobs - mu_x, 2) / (2 * pow(sig_x, 2))) + (pow(y_tobs - mu_y, 2) / (2 * pow(sig_y, 2))); 
  
  //calculate the weight with normalization terms and exponent 
  double weight; 
